...
public class ConfigCommand extends Command {
    ...
    public void execute(String[] commands) {
        if (commands.length == 1) {
            sendMessage("You`ll find the config files in your gameProfile directory under astronaut/config");
            return;
        }
        if (commands.length == 2)
            if ("list".equals(commands[0])) {
                String configs = "Configs: ";
                File file = new File("astronaut/");
        ...
        if (commands.length >= 3) {
            switch (commands[0]) {
                case "save":
                    Astronaut.configManager.saveConfig(commands[1]);
                    sendMessage(ChatFormatting.GREEN + "Config '" + commands[1] + "' has been saved.");
                    return;
                case "load":
                    if (Astronaut.configManager.configExists(commands[1])) {
                        Astronaut.configManager.loadConfig(commands[1]);
