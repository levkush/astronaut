package dev.levkush.astronaut.features.modules.client;

import com.mojang.realmsclient.gui.ChatFormatting;
import dev.levkush.astronaut.Astronaut;
import dev.levkush.astronaut.event.events.ClientEvent;
import dev.levkush.astronaut.event.events.Render2DEvent;
import dev.levkush.astronaut.features.setting.Setting;
import dev.levkush.astronaut.util.*;
...
public class HUD extends Module {
    ...
    private final dev.levkush.astronaut.util.Timer timer = new Timer();
    private final Map<String, Integer> players = new HashMap<>();
    public Setting<String> command = register(new Setting("Command", "Astronaut"));
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.waterMark.getValue().booleanValue()) {
            String string = this.command.getPlannedValue() + " v0.0.1";
        ...
        if (this.arrayList.getValue().booleanValue())
            if (this.renderingUp.getValue().booleanValue()) {
                if (this.renderingMode.getValue() == RenderingMode.ABC) {
                    for (int k = 0; k < Astronaut.moduleManager.sortedModulesABC.size(); k++) {
                        String str = Astronaut.moduleManager.sortedModulesABC.get(k);
                    ...
                    for (int k = 0; k < Astronaut.moduleManager.sortedModules.size(); k++) {
                        Module module = Astronaut.moduleManager.sortedModules.get(k);
        ...
        if (this.renderingUp.getValue().booleanValue()) {
            if (this.potions.getValue().booleanValue()) {
                ...
                for (PotionEffect potionEffect : effects) {
                    String str = Astronaut.potionManager.getColoredPotionString(potionEffect);
                    i += 10;
            ...
            if (this.speed.getValue().booleanValue()) {
                String str = grayString + "Speed " + ChatFormatting.WHITE + Astronaut.speedManager.getSpeedKpH() + " km/h";
                i += 10;
            ...
            if (this.tps.getValue().booleanValue()) {
                String str = grayString + "TPS " + ChatFormatting.WHITE + Astronaut.serverManager.getTPS();
                i += 10;
                this.renderer.drawString(str, (width - this.renderer.getStringWidth(str) - 2), (height - 2 - i), (ClickGui.getInstance()).rainbow.getValue().booleanValue() ? (((ClickGui.getInstance()).rainbowModeA.getValue() == ClickGui.rainbowModeArray.Up) ? ColorUtil.rainbow(counter1[0] * (ClickGui.getInstance()).rainbowHue.getValue().intValue()).getRGB() : ColorUtil.rainbow((ClickGui.getInstance()).rainbowHue.getValue().intValue()).getRGB()) : this.color, true);
                counter1[0] = counter1[0] + 1;
            }
            String fpsText = grayString + "FPS " + ChatFormatting.WHITE + Minecraft.debugFPS;
            String str1 = grayString + "Ping " + ChatFormatting.WHITE + Astronaut.serverManager.getPing();
        ...
        String direction = this.direction.getValue().booleanValue() ? Astronaut.rotationManager.getDirection4D(false) : "";
    ...
    public void renderLag() {
        int width = this.renderer.scaledWidth;
        if (Astronaut.serverManager.isServerNotResponding()) {
            String text = ChatFormatting.RED + "Server not responding " + MathUtil.round((float) Astronaut.serverManager.serverRespondingTime() / 1000.0F, 1) + "s.";
    ...
    public void onLoad() {
        Astronaut.commandManager.setClientMessage(getCommandMessage());
    }
