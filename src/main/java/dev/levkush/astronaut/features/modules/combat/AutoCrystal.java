package dev.levkush.astronaut.features.modules.combat;
...
import dev.levkush.astronaut.features.modules.misc.AutoGG;
import dev.levkush.astronaut.util.*;
...
public class AutoCrystal
        extends Module {
    private final dev.levkush.astronaut.util.Timer placeTimer = new dev.levkush.astronaut.util.Timer();
    private final dev.levkush.astronaut.util.Timer breakTimer = new dev.levkush.astronaut.util.Timer();
    private final dev.levkush.astronaut.util.Timer preditTimer = new dev.levkush.astronaut.util.Timer();
    private final dev.levkush.astronaut.util.Timer manualTimer = new Timer();
    ...
    public void onCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue().booleanValue()) {
            ...
            if (AutoGG.getINSTANCE().isOn()) {
                AutoGG autoGG = (AutoGG) Astronaut.moduleManager.getModuleByName("AutoGG");
    ...
    EntityPlayer getTarget() {
        ...
        for (EntityPlayer entity : AutoCrystal.mc.world.playerEntities) {
            if (AutoCrystal.mc.player == null || AutoCrystal.mc.player.isDead || entity.isDead || entity == AutoCrystal.mc.player || Astronaut.friendManager.isFriend(entity.getName()) || entity.getDistance(AutoCrystal.mc.player) > 12.0f)
                continue;
            this.armorTarget = false;
