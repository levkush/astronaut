...
import dev.levkush.astronaut.event.events.UpdateWalkingPlayerEvent;
import dev.levkush.astronaut.features.setting.Setting;
import dev.levkush.astronaut.util.DamageUtil;
import dev.levkush.astronaut.util.EntityUtil;
import dev.levkush.astronaut.util.MathUtil;
...
public class Killaura extends Module {
    ...
    private void doKillaura() {
        ...
        int wait = !this.delay.getValue().booleanValue() ? 0 : (int) (DamageUtil.getCooldownByWeapon(mc.player) * (this.tps.getValue().booleanValue() ? Astronaut.serverManager.getTpsFactor() : 1.0F));
        ...
        if (target == null)
            return;
        if (this.rotate.getValue().booleanValue())
            Astronaut.rotationManager.lookAtEntity(target);
